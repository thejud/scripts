...
def print_lines(filename: str, lines: List[str], linecount: int, print_header: bool, print_separator: bool) -> None:
    if print_header:
        print(f'==> {filename} <==')
...
def process_input(args):
    ...
    for filename in args.files:
        ...
        print_lines(filename, lines, args.linecount, not args.no_header and multiple_files, args.separator)

        if filename != 'stdin':
            file.close()

def main():
    parser = argparse.ArgumentParser(description='Print the first and last N lines of a file or stdin.')
    parser.add_argument('-n', '--linecount', type=int, default=5, help='Number of lines to print at the beginning and end of each file.')
    parser.add_argument('-H', '--no-header', action='store_true', help='Suppress printing the header between files.')
    parser.add_argument('-s', '--separator', action='store_true', help='Print a separator ("...") between the head and tail of each file.')
    parser.add_argument('files', nargs='*', help='Files to process. If none provided, read from stdin.')
